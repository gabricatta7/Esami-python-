...
with open("1.txt","w",encoding="utf-8") as o:
    scontro = []
...
### punto 4 ###
with open("4.txt","w",encoding="utf-8") as o:
    goal_contro = {}
    for i in range(len(giocatori)):
        if i%2==0:
            goal_contro[giocatori[i]] = []
    for i in goal_contro:
        for j in range(len(dati)):
            if j%3 ==0: # sono nel caso della partita (sqd1-sqd2)
                Squad1 = dati[j].get("partita").split("-")[0]
                Squad2 = dati[j].get("partita").split("-")[1]
                goalSquad1 = int(dati[j + 1].get("punteggio").split("-")[0])
                goalSquad2 = int(dati[j + 1].get("punteggio").split("-")[1])
                goaltotali = goalSquad1+goalSquad2
            if len(dati[j])>1:
                for k in range(goaltotali):
                    if i in rosa.get(Squad1):
                        if Squad2 not in goal_contro[i] and i == dati[j][k]:
                            goal_contro[i].append(Squad2)
                    if i in rosa.get(Squad2):
                        if Squad1 not in goal_contro[i] and i == dati[j][k]:
                            goal_contro[i].append(Squad1)
    for i in goal_contro:
        o.write(i+ str(goal_contro.get(i))+"\n")
### punto 5 ### trovare il giocatore che ha più doppiette
with open("5.txt","w",encoding="utf-8") as o:
    doppiette = {}
    for i in range(len(giocatori)):
        if i%2==0:
            doppiette[giocatori[i]] = []
    print(doppiette)
    ...
    for i in doppiette:
        ...
        for j in range(len(dati)):
            if len(dati[j])>1:
                conta = dati[j].count(i)
                if conta>=2:
                    c += 1
        doppiette[i] = c
    print(doppiette)
    max = max(doppiette.values())
    o.write("i giocatori/e con più doppiette:\n")
    for i in doppiette.keys():
        if doppiette.get(i) == max:
            o.write(i+"["+ str(max) +"]"+"\n")